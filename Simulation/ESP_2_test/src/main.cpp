...
void onCommand(char* topic, byte* payload, unsigned int length) {
    ...
    if (!actuators.masterRelay && actuators.mode == "OVERRIDE") {
    ...
    applyRelays();
    Serial.printf("[CMD] mode=%s lights=%d ac=%d master=%d setpoint=%.1f\n",
        actuators.mode.c_str(), actuators.lights, actuators.ac,
        actuators.masterRelay, actuators.acSetpoint);
...
void taskReadSensors(void* param) {
    while (true) {
        ...
        if (!actuators.masterRelay && actuators.mode == "AUTO") {
            ...
            bool changed = false;
            if (actuators.lights != shouldLight) { actuators.lights = shouldLight; changed = true; }
            if (actuators.ac     != shouldAC)    { actuators.ac     = shouldAC;    changed = true; }
